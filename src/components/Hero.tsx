...
// Статичный фон в обычном файле — отдаётся сразу в HTML (быстрый LCP),
// не через client-fetch/base64. Менять фон/положение — через админку.
...
export default function Hero({ onQuoteClick, onUploadClick }: HeroProps) {
  ...
  return (
    ...
    >
      {/* Предзагрузка LCP-картинки — React поднимет <link> в <head> */}
      <link rel="preload" as="image" href={HERO_IMG} fetchPriority="high" />

      {/* Overlay для читаемости текста (сохранён прежний вид: коричневый градиент 60%) */}
      <div
        className="absolute inset-0"
        style={{ background: "linear-gradient(to right, rgba(114,84,64,0.6), transparent)" }}
      />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              ...
              <a
                ...
                className="flex items-center gap-2 border border-slate-400 text-slate-200 hover:border-slate-200 hover:text-white px-6 py-3.5 font-medium text-base transition-colors min-h-[48px]"
              >
                <Phone size={20} />
                Позвонить
              </a>
            ...
            <div className="flex flex-wrap gap-2">
              {badges.map((b) => (
                <span
                  key={b}
                  className="bg-slate-800/80 border border-slate-700 text-slate-200 text-xs px-3 py-1.5"
                >
                  {b}
                </span>
              ))}
          ...
          <div className="hidden lg:flex flex-col gap-6">
            <div className="bg-slate-800/90 border border-slate-700 p-8">
              ...
              <div className="text-slate-300 text-sm mt-1">Всегда в наличии в Алматы</div>
            </div>
            <div className="bg-slate-800/90 border border-slate-700 p-8">
              ...
              <div className="text-slate-300 text-sm mt-1">Отправьте список — ответим быстро</div>
            </div>
          </div>
        </div>
      </div>
